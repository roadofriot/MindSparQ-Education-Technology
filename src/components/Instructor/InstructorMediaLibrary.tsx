...
import {
  Folder, Image as ImageIcon, Film, FileText, HardDrive,
  Search, Upload, Plus, Trash2, Copy, Check, Eye, ExternalLink,
  Tag, Filter, X, Sparkles, Download, Video, Image as ImageIcon2
} from 'lucide-react';
...
export const InstructorMediaLibrary: React.FC<InstructorMediaLibraryProps> = ({ onSelectMedia }) => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
        <div>
          <h2 className="text-base font-bold text-slate-900 flex items-center space-x-2">
            ...
            <span>Instructor Media Library</span>
          </h2>
          <p className="text-xs text-slate-500 mt-0.5">
            Store and manage your images, videos, documents, and Google Drive assets.
          </p>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl border border-slate-200 max-w-md w-full p-6 space-y-4 shadow-2xl relative text-slate-900">
            ...
            <form onSubmit={handleCreateFolder} className="space-y-3 text-xs">
              <div>
                <label className="block font-semibold text-slate-700 mb-1">Folder Name *</label>
                <input
                  type="text"
                  required
                  placeholder="e.g. Course Materials 2026"
                  value={newFolderName}
